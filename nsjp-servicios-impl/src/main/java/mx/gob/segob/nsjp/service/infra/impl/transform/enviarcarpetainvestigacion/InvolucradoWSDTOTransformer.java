...
import mx.gob.segob.nsjp.ws.cliente.enviarcarpetainvestigacion.FuncionarioWSDTO;
import mx.gob.segob.nsjp.ws.cliente.enviarcarpetainvestigacion.InvolucradoWSDTO;
import mx.gob.segob.nsjp.ws.cliente.enviarcarpetainvestigacion.LugarWSDTO;
...
import mx.gob.segob.nsjp.ws.cliente.enviarcarpetainvestigacion.SeniaParticularWSDTO;
...
public class InvolucradoWSDTOTransformer {
    ...
    public static InvolucradoDTO transforma(mx.gob.segob.nsjp.dto.involucrado.InvolucradoWSDTO involucradoWSDTO){
        ...
        if (involucradoWSDTO.getDelitosCometidos()!=null && !involucradoWSDTO.getDelitosCometidos().isEmpty()){
            DelitoDTO novoDel = null;
            for (mx.gob.segob.nsjp.dto.expediente.DelitoWSDTO del:involucradoWSDTO.getDelitosCometidos()) {
                novoDel = new DelitoDTO();
                CatDelitoDTO catDelitoDTO = new CatDelitoDTO();
                catDelitoDTO.setCatDelitoId(del.getIdCatDelito());
                if(del.getClaveInterInstitucional() != null){
                	catDelitoDTO.setClaveInterInstitucional(del.getClaveInterInstitucional());
                }
                novoDel.setCatDelitoDTO(catDelitoDTO);
                novoDel.setEsPrincipal(del.isEsPrincipal());
                novoDel.setEsProbable(del.isEsProbable());
                involucradoDTO.addDelitoCometido(novoDel);
            }
        }
        ...
        if(involucradoWSDTO != null && involucradoWSDTO.getEsVivo() != null){
        	if(involucradoWSDTO.getEsVivo().equals(ES_VIVO)){
        ...
        List<mx.gob.segob.nsjp.dto.involucrado.DetencionWSDTO> detenciones = involucradoWSDTO.getDetenciones();
		if(detenciones!=null && !detenciones.isEmpty()){
			List<DetencionDTO> dentencionesDTO = new ArrayList<DetencionDTO>();
			for (mx.gob.segob.nsjp.dto.involucrado.DetencionWSDTO detencionDTO : detenciones) {
				dentencionesDTO.add(InvolucradoWSDTOTransformer.transforma(detencionDTO));
			}
			involucradoDTO.setDetenciones(dentencionesDTO);
		}
		
		if (involucradoWSDTO.getIdsDetenidos() != null
				&& !involucradoWSDTO.getIdsDetenidos().isEmpty()) {
			involucradoDTO.setIdsDetenidos(
					involucradoWSDTO.getIdsDetenidos());
		}
		
		if (involucradoWSDTO.getFotoDelElemento() != null) {
			involucradoDTO.setFotoDelElemento(ExpedienteWSDTOTransformer
					.transforma(involucradoWSDTO.getFotoDelElemento()));
		}
		
        logger.debug("involucradoDTO.folioElemento :: " + involucradoDTO.getFolioElemento());
        return involucradoDTO;
    }

    public static DetencionDTO transforma(mx.gob.segob.nsjp.dto.involucrado.DetencionWSDTO detencionWSDTO){
    	DetencionDTO detencionDTO = new DetencionDTO();
    	detencionDTO.setFechaInicioDetencion(detencionWSDTO.getFechaInicioDetencion());
		detencionDTO.setFechaInicioDetencion(detencionWSDTO.getFechaInicioDetencion());
    	detencionDTO.setHoraInicioDetencion(detencionWSDTO.getHoraInicioDetencion());
    	detencionDTO.setFechaFinDetencion(detencionWSDTO.getStrFechaFinDetencion());
		detencionDTO.setFechaFinDetencion(detencionWSDTO.getFechaFinDetencion());
    	detencionDTO.setHoraFinDetencion(detencionWSDTO.getHoraFinDetencion());
    	detencionDTO.setMotivoDetencion(detencionWSDTO.getMotivoDetencion());
    	detencionDTO.setFechaRecepcionDetencion(detencionWSDTO.getStrFechaRecepcionDetencion());
    	detencionDTO.setFechaRecepcionDetencion(detencionWSDTO.getFechaRecepcionDetencion());
    	detencionDTO.setHoraRecepcionDetencion(detencionWSDTO.getHoraRecepcionDetencion());
    	detencionDTO.setObservaciones(detencionWSDTO.getObservaciones());
    	detencionDTO.setLugarDetencionProvicional(detencionWSDTO.getLugarDetencionProvicional());
    	
    	if(detencionWSDTO.getLugarDetencionWSDTO()!=null){
			detencionDTO.setLugarDetencionDTO(InvolucradoWSDTOTransformer
					.transforma(detencionWSDTO.getLugarDetencion()));
    	}
    	
    	if(detencionWSDTO.getQuienDetuvoAsFun()!=null){
    		detencionDTO.setFuncionarioByFuncionarioDetiene(InvolucradoWSDTOTransformer
					.transforma(detencionWSDTO.getQuienDetuvoAsFun()));
    	}
    	
    	if(detencionWSDTO.getQuienDetuvoAsInv()!=null){
    		//Se pasa solo el folio del elemento y la institucion que presenta
    		InvolucradoDTO involucradoDTO = new InvolucradoDTO();
			involucradoDTO.setFolioElemento(detencionWSDTO.getQuienDetuvoAsInv()
					.getFolioElemento());
			if(detencionWSDTO.getQuienDetuvoAsInv()
					.getInstitucionPresenta()!=null){
				involucradoDTO.setInstitucionPresenta(new ConfInstitucionDTO(detencionWSDTO.getQuienDetuvoAsInv()
					.getInstitucionPresenta()));
			}
    ...
    public static LugarDTO transforma(mx.gob.segob.nsjp.dto.domicilio.LugarWSDTO lugarSWDTOX){
    	LugarDTO lugarDTO = new LugarDTO();
    	ExpedienteWSDTOTransformer.transforma(lugarDTO, lugarSWDTOX);
    	return lugarDTO;
    }    
    
    public static FuncionarioDTO transforma(mx.gob.segob.nsjp.dto.funcionario.FuncionarioWSDTO funcionarioWSDTO){
    	FuncionarioDTO funcionarioDTO = new FuncionarioDTO();
    	ExpedienteWSDTOTransformer.transforma(funcionarioWSDTO, funcionarioDTO);
    ...
    public static OrganizacionDTO transforma(mx.gob.segob.nsjp.dto.organizacion.OrganizacionWSDTO organizacionWSDTO){
        ...
        if (organizacionWSDTO.getCalidad() != null){
        	organizacionDTO.setCalidadDTO(transforma(organizacionWSDTO.getCalidad()));
        }
        
        if (organizacionWSDTO.getDomicilioWSDTO() != null){
        	organizacionDTO.setDomicilioDTO(transforma(organizacionWSDTO.getDomicilioWSDTO()));
        }
    ...
    public static InvolucradoWSDTO transforma(InvolucradoDTO involucradoDto){
        ...
        if (involucradoDto.getDelitosCometidos()!=null && !involucradoDto.getDelitosCometidos().isEmpty()) {
            DelitoWSDTO niuDel = null;
            for (DelitoDTO del : involucradoDto.getDelitosCometidos()){
                niuDel = new DelitoWSDTO();
                niuDel.setClaveDelito(del.getCatDelitoDTO().getClaveDelito());
                niuDel.setEsGrave(del.getCatDelitoDTO().getEsGrave());
                niuDel.setEsPrincipal(del.getEsPrincipal());
                niuDel.setEsProbable(del.getEsProbable());
                niuDel.setIdCatDelito(del.getCatDelitoDTO().getCatDelitoId());
                niuDel.setNombreDelito(del.getCatDelitoDTO().getNombre());
                niuDel.setClaveInterInstitucional(del.getCatDelitoDTO().getClaveInterInstitucional());
                involucradoWsdto.getDelitosCometidos().add(niuDel);
            }
        }
		...
		if (involucradoDto.getEsVivo() != null
				&& involucradoDto.getEsVivo().equals(
		...
		if(detenciones!=null && !detenciones.isEmpty()){
			for (DetencionDTO detencionDTO : detenciones) {
				involucradoWsdto.getDetenciones().add(
						InvolucradoWSDTOTransformer.transforma(detencionDTO));
			}
		}
		
		if (involucradoDto.getIdsDetenidos() != null
				&& !involucradoDto.getIdsDetenidos().isEmpty()) {
			involucradoWsdto.getIdsDetenidos().addAll(
					involucradoDto.getIdsDetenidos());
		}
		
		if (involucradoDto.getFotoDelElemento() != null) {
			involucradoWsdto.setFotoDelElemento(ExpedienteWSDTOTransformer
					.transforma(involucradoDto.getFotoDelElemento()));
		}
    ...
    public static DetencionWSDTO transforma(DetencionDTO detencionDTO){
    	...
    	detencionWSDTO.setStrFechaInicioDetencion(detencionDTO.getStrFechaInicioDetencion());
    	detencionWSDTO.setFechaInicioDetencion(WsTransformer.
                transformFecha(detencionDTO.getFechaInicioDetencion()));
    	detencionWSDTO.setHoraInicioDetencion(detencionDTO.getHoraInicioDetencion());
    	detencionWSDTO.setStrFechaFinDetencion(detencionDTO.getstrfechaFinDetencion());
		detencionWSDTO.setFechaFinDetencion(WsTransformer
				.transformFecha(detencionDTO.getFechaFinDetencion()));
    	detencionWSDTO.setHoraFinDetencion(detencionDTO.getHoraFinDetencion());
    	detencionWSDTO.setMotivoDetencion(detencionDTO.getMotivoDetencion());
    	detencionWSDTO.setStrFechaRecepcionDetencion(detencionDTO.getStrFechaRecepcionDetencion());
		detencionWSDTO.setFechaRecepcionDetencion(WsTransformer
				.transformFecha(detencionDTO.getFechaRecepcionDetencion()));
    	detencionWSDTO.setHoraRecepcionDetencion(detencionDTO.getHoraRecepcionDetencion());
    	detencionWSDTO.setObservaciones(detencionDTO.getObservaciones());
    	detencionWSDTO.setLugarDetencionProvicional(detencionDTO.getLugarDetencionProvicional());
    	...
    	if(detencionDTO.getFuncionarioByFuncionarioDetiene()!=null){
    		detencionWSDTO.setQuienDetuvoAsFun(InvolucradoWSDTOTransformer
					.transforma(detencionDTO.getFuncionarioByFuncionarioDetiene()));
    	}
    	
    	if(detencionDTO.getInvolucradoDTO()!=null){
    		//Se pasa solo el folio del elemento
    		InvolucradoWSDTO involucradoWSDTO = new InvolucradoWSDTO();
			involucradoWSDTO.setFolioElemento(detencionDTO.getInvolucradoDTO()
					.getFolioElemento());
			if(detencionDTO.getInvolucradoDTO()
					.getInstitucionPresenta()!=null){
				involucradoWSDTO.setInstitucionPresenta(detencionDTO.getInvolucradoDTO()
					.getInstitucionPresenta().getConfInstitucionId());
			}
			detencionWSDTO.setQuienDetuvoAsInv(involucradoWSDTO);
    	}
    ...
    public static FuncionarioWSDTO transforma(FuncionarioDTO funcionarioDTO){
    	FuncionarioWSDTO funcionarioWSDTO = new FuncionarioWSDTO();
    	ExpedienteWSDTOTransformer.transforma(funcionarioDTO, funcionarioWSDTO);
    	funcionarioWSDTO.setNombre(funcionarioDTO.getNombreFuncionario());
    	funcionarioWSDTO.setApellidoPaterno(funcionarioDTO.getApellidoPaternoFuncionario());
    	funcionarioWSDTO.setApellidoMaterno(funcionarioDTO.getApellidoMaternoFuncionario());
    ...
    public static OrganizacionWSDTO transforma(OrganizacionDTO organizacionDTO){
        ...
        if (organizacionDTO.getCalidadDTO()!= null){
        	organizacionWSDTO.setCalidad(transforma(organizacionDTO.getCalidadDTO()));
        }
        
        if(organizacionDTO.getDomicilioDTO() != null){
        	organizacionWSDTO.setDomicilioWSDTO(transforma(organizacionDTO.getDomicilioDTO()));
        }
