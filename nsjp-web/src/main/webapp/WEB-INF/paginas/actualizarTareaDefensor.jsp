...
function grid2(){
				...
				ondblClickRow: function(rowID) {
					...
					idDefensor=rowID;
						...
						cargaActividades();
							...
							alertDinamico("Falta seleccionar la fecha");
						}
	...
	function cargaActividades(){
		...
		if (reloadGridcarga==true) {
			jQuery("#tabgridActividad").jqGrid('setGridParam', {url:'<%= request.getContextPath()%>/ConsultaTareasDefensor.do'});
			
			 $("#tabgridActividad").trigger("reloadGrid"); 
		} else{
			jQuery("#tabgridActividad").jqGrid({ 
				url:'<%= request.getContextPath()%>/ConsultaTareasDefensor.do?id='+idDefensor+'&dia='+varDia,
				data:"",
				datatype: "xml", 
...
jQuery("#tabgridActividad").jqGrid('editRow',rowID);
				...
				onSelectRow: function(rowID) {
					idCarga=rowID;
					//jQuery("#tabgridActividad").jqGrid('restoreRow',idCarga);
					}
	...
	function asignarCargaTrabajo() {
			...
			} else { alertDinamico("Falta seleccionar un defensor y la tarea correspondiente");

			}
		
		   $.ajax({
		     type: 'POST',
		     url: '<%= request.getContextPath()%>/AsignarCargaTrabajoActualizado.do',
			 data: param,
			 dataType: 'xml',
