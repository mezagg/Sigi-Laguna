...
var reloadGridEvidencia = false;
		//Llena el grid con los resultados de la busqueda, pasa como parametros la evidencia y una a 5 palabras clave
						...
						if (validaEvidencia==true){
							if (reloadGridEvidencia) {
								  ...
								  jQuery("#tablaBuscarExpedientePorEvidencia").jqGrid(
											{ url:'<%= request.getContextPath() %>/buscarExpedientePorEvidenciaGeneral.do', 						
		  ...
		  function detEvi(tr){
				$.newWindow({id:"iframewindowDetalleExpEvidencia", statusBar: true, posx:200,posy:50,width:806,height:500,title:"Detalle de Caso", type:"iframe"});
		...
		//Funcion que valida si los campos estan llenos al enviar 
			function validaCamposEvidencia(){
				...
				if ($('#palabraClave').val()==''&& $('#palabraClave2').val()==''&& $('#palabraClave3').val()==''&& $('#palabraClave4').val()==''&& $('#palabraClave5').val()==''){
						customAlert("Se debe capturar al menos una plabra clave");
		  ...
		  $.ajax({
	    	  ...
	    	  success: function(xml){
	    		  ...
	    		  $(xml).find('catEvidencia').each(function(){
					$('#catEvidencia').append('<option value="' + $(this).find('valor').text() + '">' + $(this).find('valor').text() + '</option>');
