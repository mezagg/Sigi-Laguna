...
<title>Ingresar tema de linea de investigaci&oacute;n</title>	   
	   <script type="text/javascript" src="<%=request.getContextPath()%>/js/comun.js?n=1"></script>
	   <script type="text/javascript">
			
			function guarda() {
				var recuperaTextoDeTema=$('.ckeditor_TemaLineaDeInv').val();
				window.parent.$("#textTema").val(recuperaTextoDeTema);
			}
			
			function recuperaDescripcionTema()
			{
				return $('.ckeditor_TemaLineaDeInv').val();
			}
			
			function recuperaDescripcionTemaBD()
			{
				var lsNarrativa="";
				lsNarrativa="gsNarrativa="+$('.ckeditor_TemaLineaDeInv').val();
	  			return lsNarrativa;
			}
...
<body>
      <textarea class="ckeditor_TemaLineaDeInv" cols="100" id="editorTemaInv" name="editorTemaInv" rows="10" ></textarea>
</body>
</html>
...
var config = {					
	...
	if(window.parent.banderaNarrativa==1)
		{
			$("#editorTemaInv").val('<%= request.getSession().getAttribute("narrativa")%>');
		}
	
		//cargaTiposNarrativa();
    	//enableControls('0');
       
		$('.ckeditor_TemaLineaDeInv').ckeditor(config);
