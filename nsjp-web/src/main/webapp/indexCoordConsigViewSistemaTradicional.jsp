...
<head>
	...
	var cargaPrimeraConsignadores = true;
	var tipoDeConsultaDeExpedientes = "nuevos";
	...
	$(document).ready(function() {
		...
		cargaGridExpedientesPorEstatus("<%=EstatusExpediente.ABIERTO.getValorId()%>","","",<%=Roles.COORDINADOR_CONSIGNACION.getValorId()%>);
	...
	*/
	...
	,	center: {
		...
		,	onresize_end:			function () { 
										$("#gridExpedientesPorEstatus").setGridWidth($("#mainContent").width() - 2, true);
										$("#gridConsignadores").setGridWidth($("#mainContent").width() - 2, true);
									}
		}
	};
	...
	*/
	...
	/*
	*Funcion que consulta consignadores por distrito
	*/
	function cargarGridConsignadoresPorDistrito(estatus,fechaIni,fechaFin,idRol,idConsignador){
		
		var parametros = "";
		parametros += 'idRol=' + <%=Roles.CONSIGNADOR.getValorId()%>;
		parametros += '&idDistrito=' + <%= idDistrito %>;
		
		jQuery("#gridConsignadores").jqGrid('setGridParam', { 
			ondblClickRow: function(id) {
				cargaGridExpedientesPorEstatus(estatus,fechaIni,fechaFin,idRol,idConsignador,id);
			} })
                .trigger('reloadGrid');

		if(cargaPrimeraConsignadores == true){

			jQuery("#gridConsignadores").jqGrid({ 
				url:'<%=request.getContextPath()%>/consultarConsignadores.do?'+ parametros + '',
				datatype: "xml", 
				colNames:['Nombre del consignador'], 
				colModel:[ 	{name:'Nombre',index:'1', width:800, align:"center"},
						],
				pager: jQuery('#pagerGridConsignadores'),
				rowNum:10,
				rowList:[10,20,30],
				autowidth: true,
				autoheight:true,
				...
				sortorder: "desc"
			}).navGrid('#pagerGridConsignadores',{edit:false,add:false,del:false});

			cargaPrimeraConsignadores = false;
		}else{
			jQuery("#gridConsignadores").jqGrid('setGridParam', {url:'<%=request.getContextPath()%>/consultarConsignadores.do?'+ parametros + '',data: parametros,datatype: "xml" });
			$("#gridConsignadores").setGridParam({page:"1"}).trigger("reloadGrid");
			$("#gridConsignadores").trigger("reloadGrid");
		}
		
		//Se limipia el grid de Expedientes
		$("#gridExpedientesPorEstatus").jqGrid("clearGridData", true);
	...
	*/
	function ocultaMuestraGrids(nombreGrid){
		...
		if(nombreGrid == "gridConsignadores"){
			$("#divGridConsignadores").show();
			$("#divGridExpedientePorEstatus").show();
			
		}
		if(nombreGrid == "gridExpedientesPorEstatus"){
			$("#divGridConsignadores").hide();
			$("#divGridExpedientePorEstatus").show();
		}
	}
    ...
    */
    function consultaDenunciaTradicional(id,estatus,fechaIni,fechaFin,idRol,idConsignador) {
        ...
        var activarAsignacionAConsignador = true;
		...
		parametros += '&activarAsignacionAConsignador='+activarAsignacionAConsignador;
    ...
    */
	...
	function recargarBadejaIndexCoordinadorConsignador(identificador){
		switch(identificador){
			case 'nuevos':
				cargaGridExpedientesPorEstatus("<%=EstatusExpediente.ABIERTO.getValorId()%>","","",<%=Roles.COORDINADOR_CONSIGNACION.getValorId()%>);
				break;
			case 'enProceso':
				cargaGridExpedientesPorEstatus("<%=EstatusExpediente.POR_ATENDER.getValorId()%>,<%=EstatusExpediente.EN_PROCESO.getValorId()%>,<%=EstatusExpediente.DEVUELTO.getValorId()%>","","",<%=Roles.COORDINADOR_CONSIGNACION.getValorId()%>);
				break;
			case 'cerrado':
				cargaGridExpedientesPorEstatus("<%=EstatusExpediente.NEAP.getValorId()%>,<%=EstatusExpediente.CONSIGNADO.getValorId()%>","","",<%=Roles.COORDINADOR_CONSIGNACION.getValorId()%>);
				break;
		}		
	}
		...
		<div id="accordionmenuprincipal">
			...
			<div>
				<ul id="seccion2tree" class="filetree">	
					<li class="closed" ><span id="nuevos" class="folder" onclick='ocultaMuestraGrids("gridExpedientesPorEstatus");
											    cargaGridExpedientesPorEstatus("<%=EstatusExpediente.ABIERTO.getValorId()%>","","",<%=Roles.COORDINADOR_CONSIGNACION.getValorId()%>);asignaValorTipoConsulta("nuevos")'>Nuevos</span>
						<ul></ul>
					</li>
				</ul>		
				
				<ul id="seccion3tree" class="filetree">	
					<li class="closed" ><span id="enproceso" class="folder">En proceso</span>
						<ul>				
							<li>						
								<span class='file'>
									<a onclick='ocultaMuestraGrids("gridExpedientesPorEstatus");
											    cargaGridExpedientesPorEstatus("<%=EstatusExpediente.POR_ATENDER.getValorId()%>,<%=EstatusExpediente.EN_PROCESO.getValorId()%>,<%=EstatusExpediente.DEVUELTO.getValorId()%>","","",<%=Roles.COORDINADOR_CONSIGNACION.getValorId()%>);asignaValorTipoConsulta("enProceso")'>
											    De todos los consignadores
									</a>
							...
							<li>
								<span class='file'>
									<a onclick='ocultaMuestraGrids("gridConsignadores");
										    cargarGridConsignadoresPorDistrito("<%=EstatusExpediente.POR_ATENDER.getValorId()%>,<%=EstatusExpediente.EN_PROCESO.getValorId()%>,<%=EstatusExpediente.DEVUELTO.getValorId()%>","","",<%=Roles.CONSIGNADOR.getValorId()%>)'>
										    Por consignador
									</a>							
								</span>
							</li>
				...
				<ul id="seccion4tree" class="filetree">	
					<li class="closed" ><span id="cerrados" class="folder">Cerrados</span>
						<ul>				
							<li>						
								<span class='file'>
									<a onclick='ocultaMuestraGrids("gridExpedientesPorEstatus");
											    cargaGridExpedientesPorEstatus("<%=EstatusExpediente.NEAP.getValorId()%>,<%=EstatusExpediente.CONSIGNADO.getValorId()%>","","",<%=Roles.COORDINADOR_CONSIGNACION.getValorId()%>);asignaValorTipoConsulta("cerrado")'>
											    De todos los consignadores
									</a>
								</span>
							</li>
							<li>
								<span class='file'>
									<a onclick='ocultaMuestraGrids("gridConsignadores");
										    cargarGridConsignadoresPorDistrito("<%=EstatusExpediente.NEAP.getValorId()%>,<%=EstatusExpediente.CONSIGNADO.getValorId()%>","","",<%=Roles.CONSIGNADOR.getValorId()%>)'>
										    Por consignador
									</a>					
								</span>
							</li>
		...
		<div class="ui-layout-content">
			<div class="ui-layout-north">
				...
				<table width="100%" height="100%" border="0">
				  <tr height="50%">
				    <td>
						<div id="divGridConsignadores">
							<table id="gridConsignadores"></table>
							<div id="pagerGridConsignadores" style=" vertical-align: top;"></div>
						</div>			
					</td>
				  </tr>
				  <tr height="50%">
