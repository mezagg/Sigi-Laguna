	...
	*/
	...
	,	center: {
		...
		,	onresize_end:			function () { 
			...
			ajustarGridAlCentro($("#gridDocumentosRecibidos"));
			ajustarGridAlCentro($("#gridResguardoEvidencia"));
			ajustarGridAlCentro($("#gridAsignacionesRecibidasEnProceso"));
			ajustarGridAlCentro($("#gridAsignacionesRecibidasNoAtendidas")); 										 
									}
		}
	};
...
/////////////////////////////////////////////////////////////////////////FUNCIONALIDAD PARA ASIGNACIONES RECIBIDAS ////////////////////////////////////////////////////////////////
	...
	function cargaGridAsignacionesRecibidasNoAtendidas(){
		...
		if(firstGridAsignacionesRecibidasNoAtendidas == true){
			...
			jQuery("#gridAsignacionesRecibidasNoAtendidas").jqGrid({ 
				...
				datatype: "xml", 
				colNames:['Nombre del Solicitante','Tipo de Asignaci&oacute;n','N&uacute;mero de Expediente','N&uacute;mero de Caso','Fecha L&iacute;mite','Acuse' ], 
				colModel:[ 	{name:'NombreSolicitante',index:'1', width:180},
							...
							{name:'FechaLimite',index:'5', width:60, searchoptions:{dataInit:function(elem){$(elem).datepicker();}, attr:{title:'Select Date'}} },
			...
			$("#gview_gridAsignacionesRecibidasEnProceso .ui-jqgrid-bdiv").css('height', '450px');
   ...
   */
   function cargaGridAsignacionesRecibidasEnProceso(){
	   ...
	   if(firstGridAsignacionesRecibidasEnProceso == true){
		   ...
		   jQuery("#gridAsignacionesRecibidasEnProceso").jqGrid({ 
				...
				datatype: "xml", 
				colNames:['N&uacute;mero Expediente','N&uacute;mero de Caso','Fecha L&iacute;mite','Fecha &Uacute;ltima Modificaci&oacute;n','Documento creado','idExp','numExpid'], 
				colModel:[ 	{name:'numeroExpediente',index:'1', width:220,align:'center'}, 
							...
							{name:'documentoCreado',index:'5', width:160,align:'center',hidden:true},
				...
				sortorder: "desc",
				ondblClickRow: function(rowid){
					var ret2 = jQuery("#gridAsignacionesRecibidasEnProceso").jqGrid('getRowData',rowid);
    ...
    *Funcion que carga el grid de asignaciones periciales en proceso
    */
    function cargaGridAsignacionesRecibidasCerradas(){

	   if(firstGridAsignacionesRecibidasEnProceso== true){
 		   
 		   jQuery("#gridAsignacionesRecibidasEnProceso").jqGrid({ 
 	
 				url:'<%=request.getContextPath()%>/consultarSolicitudesPericialesPeritoEnProceso.do?estatusSolicitud=cerrado', 
 				datatype: "xml", 
 				colNames:['N&uacute;mero Expediente','N&uacute;mero de Caso','Fecha L&iacute;mite','Fecha &Uacute;ltima Modificaci&oacute;n','Documento creado','idExp','numExpid'], 
 				colModel:[ 	{name:'numeroExpediente',index:'1', width:220,align:'center'}, 
 				           	{name:'numeroCaso',index:'2', width:220,align:'center'}, 
 		   ...
 		   firstGridAsignacionesRecibidasEnProceso = false;
 			...
 			jQuery("#gridAsignacionesRecibidasEnProceso").jqGrid('setGridParam', {url:'<%= request.getContextPath()%>/consultarSolicitudesPericialesPeritoEnProceso.do?estatusSolicitud=cerrado',datatype: "xml" });
 			$("#gridAsignacionesRecibidasEnProceso").trigger("reloadGrid");			
 		}
    ...
    function muestraGridSolProceso(){
    	
    	if(firstGridAsignacionesRecibidasEnProceso== true){
  		   
  		   jQuery("#gridAsignacionesRecibidasEnProceso").jqGrid({ 
  	
  				url:'<%=request.getContextPath()%>/consultarSolicitudesPericialesPeritoEnProceso.do?estatusSolicitud=Peticion', 
  				datatype: "xml", 
  				colNames:['N&uacute;mero Expediente','N&uacute;mero de Caso','Fecha L&iacute;mite','Fecha &Uacute;ltima Modificaci&oacute;n','Documento creado','idExp','numExpid'], 
  				colModel:[ 	{name:'numeroExpediente',index:'1', width:220,align:'center'}, 
  				           	{name:'numeroCaso',index:'2', width:220,align:'center'}, 
  				...
  				ondblClickRow: function(rowid){
  					...
  					numidExpedienteDoc=ret2.numidExpediente;
  					numExpediente = ret2.numeroExpediente;
  					mostrarVisorAtencionSolicitudesPericiales(rowid,true);
  				}
  			}).navGrid('#pagerGridAsignacionesRecibidasEnProceso',{edit:false,add:false,del:false});
  		   $("#gview_gridAsignacionesRecibidasEnProceso .ui-jqgrid-bdiv").css('height', '450px');
  		   firstGridAsignacionesRecibidasEnProceso = false;
  			...
  			jQuery("#gridAsignacionesRecibidasEnProceso").jqGrid('setGridParam', {url:'<%= request.getContextPath()%>/consultarSolicitudesPericialesPeritoEnProceso.do?estatusSolicitud=Peticion',datatype: "xml" });
  			$("#gridAsignacionesRecibidasEnProceso").trigger("reloadGrid");			
  		}
    	//muestra este grid y oculta los demas
 		ocultaMuestraGrids('gridAsignacionesRecibidasEnProceso');
	...
	function mostrarVisorAtencionSolicitudesPericiales(rowId,deshabilitaActucaiones){

		params = rowId.split(",");
		documentoId = params[1];
		solicitudId = params[0];
		
		if(isEmpty(deshabilitaActucaiones)){
			deshabilitaActucaiones = false;
		}

		idWindowVisorAntencionSolPer++;
		$.newWindow({id:"iframewindowVisorAtnSolPer"+idWindowVisorAntencionSolPer, statusBar: true, posx:150,posy:30,width:1200,height:600,title:"Atenci&oacute;n de solicitudes periciales", type:"iframe"});
	    $.updateWindowContent("iframewindowVisorAtnSolPer"+idWindowVisorAntencionSolPer,'<iframe src="<%= request.getContextPath() %>/visorAtencionSolicitudesPericiales.do?solicitudId='+solicitudId+'&documentoId='+documentoId+'&deshabilitaActucaiones='+deshabilitaActucaiones+'" width="1200" height="600" />');		
...
<div>
	<ul id="seccion2tree" class="filetree">	
	   ...
	   <li><span><img src="<%=request.getContextPath()%>/resources/images/icn_folderchek.png" width="20" height="16"  /><a id="evidenciaConcluida" onclick="muestraGridSolProceso();">En Proceso</a></span></li>
	   <li><span><img src="<%=request.getContextPath()%>/resources/images/icn_folderchek.png" width="20" height="16"  /><a id="evidenciaConcluida" onclick="cargaGridAsignacionesRecibidasCerradas();">Concluidas</a></span></li>	
	</ul>
</div>
</div>
</div>
