 ...
 * Nota, las cosas con peticiones Ajax no se concideran por lo que aun no podr�a
...
/*
 * Funcion que permite solo capturar letras con '�' y '�'
 * Se debe colocar dentro de la accion onkeypressdown del textbox
 * */
 ...
 * "�Est� seguro que desea salir? Si sale y no ha guardado sus cambios estos se perder�n"
 */
...
function soloLetrasConAcentosYNumeros(e) { 
    ...
    patron =/[A-Za-zÑñáéíóúÁÉÍÓÚäëïöüÄËÏÖÜàèìòùÀÈÌÒÙ0-9,()\s]/;
...
function validaSoloLetrasConAcentosYNumeros(campo){
	var regexp = new RegExp(/^[A-Za-zÑñáéíóúÁÉÍÓÚäëïöüÄËÏÖÜàèìòùÀÈÌÒÙ0-9,()\.\s]*$/);
    aEvaluar = campo.value;
    if (!regexp.test(aEvaluar)) {
    	customAlert("Caracteres invalidos en:<br>"+aEvaluar);
        campo.value = "";
