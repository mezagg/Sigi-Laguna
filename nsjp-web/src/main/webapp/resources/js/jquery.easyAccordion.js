...
var nombreAcordeon = 'cejaDatosGenerales';
...
(function(jQuery) {
	...
	this.each(function() {
		...
		jQuery(this).find('dt').not('active').click(function(){
			...
			//var banderaComparacion = prueba(this);
			/*Comentar la linea de abajo para que no cambie de pestaña hasta validar correctamente*/
			banderaComparacion = 1;
			if(banderaComparacion == 1){
				//alert('entro');
				nombreAcordeon = jQuery(this).attr('id');
				jQuery(this).activateSlide();
				clearTimeout(timerInstance.value);
			}else{
				alert('Los datos no son validos favor de verificar');
			}
...
function prueba(componente){
	var bandera = 0;
	
	if(nombreAcordeon == 'cejaDatosGenerales'){
		//alert('Valido datos generales');
		bandera = obtenDatosGenerales();
	} else if(nombreAcordeon == 'cejaDomicilio'){
		//alert('Valido domicilio');
	} else if(nombreAcordeon == 'cejaMediaFiliacion'){
		//alert('Valido media filiacion');
	} else if(nombreAcordeon == 'cejaMediosContacto'){
		//alert('Valido medios de contacto');
	} else if(nombreAcordeon == 'cejaDocumentosIdentificacion'){
		//alert('Valido documentos de identificacion');
	} 
	return bandera;
}

function obtenDatosGenerales(){
	var bandera = 0;
	frames[0].mandaDatosPadre();
	return 1;
}

function validaDatosFormatoTipo(nombre, apPat,apMat,curp,rfc,fechaNac,edadAproximada){
	var exprCadena = /[^a-zA-Z Ññ]/;
	//var exprCadena = /[a-zA-Z ñÑ]/;
	
	if(trim(nombre)!=""){
		//alert('nombre no es vacío');
		if (nombre.match(exprCadena)){
			//alert('Es válido');			
		}else{
