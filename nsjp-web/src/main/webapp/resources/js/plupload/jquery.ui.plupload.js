(function(f,a,c,g,e){
						...
						'<div class="plupload">'+
							'<div class="ui-state-default ui-widget-header plupload_header">'+
								'<div class="plupload_header_content">'+
									'<div class="plupload_header_title">'+b("Seleccionar Archivos")+'</div>'+
									'<div class="plupload_header_text">'+b("Agrega archivos a la caja  y oprime el boton adjuntar archivos.")+'</div>'+
								'</div>'+
							'</div>'+
							'<div class="plupload_content">'+
								'<table class="plupload_filelist">'+
									'<tr class="ui-widget-header plupload_filelist_header">'+
										'<td class="plupload_cell plupload_file_name">'+b("Nombre de archivo")+'</td>'+
										'<td class="plupload_cell plupload_file_status">'+b("Estatus")+'</td>'+
										'<td class="plupload_cell plupload_file_size">'+b("Tamaño")+'</td>'+
								...
								'<table class="plupload_filelist">'+
									'<tr class="ui-widget-header ui-widget-content plupload_filelist_footer">'+
										'<td class="plupload_cell plupload_file_name">'+
											...
											'<!-- Visible -->'+
											'<a class="plupload_button plupload_add">'+b("Agregar Archivos")+'</a>'+
											'&nbsp;'+
											'<a class="plupload_button plupload_start">'+b("Adjuntar archivos")+'</a>'+
											'&nbsp;'+
											'<a class="plupload_button plupload_stop plupload_hidden">'+b("Stop Upload")+'</a>'+
											'&nbsp;'+
											'</div>'+
											...
											'<div class="plupload_progress plupload_right">'+
												...
												'</div>'+
											'</div>'+
											'<div class="plupload_cell plupload_upload_status">'+
											'</div>'+
	...
	g.widget("ui.plupload",{
		...
		_create:function(){
			...
			if(g.ui.button){this.browse_button.button({icons:{primary:"ui-icon-circle-plus"}});
			...
			this.stop_button.button({icons:{primary:"ui-icon-circle-close"}})}
				if(!this.options.buttons.browse){
					...
					j.bind("Init",function(l,m){if(!i.options.unique_names&&i.options.rename){i._enableRenaming()}
					...
					i.start_button.click(function(n){
						...
						n.preventDefault()});
					i.stop_button.click(function(n){j.stop();
					n.preventDefault()})});
					if(i.options.max_file_count){
						j.bind("FilesAdded",function(l,n){
							...
							if(p>0){
								o=n.splice(m-p,p);
								l.trigger("Error",{code:i.FILE_COUNT_ERROR,message:b("File count error."),file:o})}})}j.init();
								j.bind("FilesAdded",function(l,m){i._trigger("selected",null,{up:l,files:m});
								if(i.options.autostart){i.start()}});
								j.bind("FilesRemoved",function(l,m){i._trigger("removed",null,{up:l,files:m})});
								j.bind("QueueChanged",function(){i._updateFileList()});
								j.bind("StateChanged",function(){i._handleState()});
								j.bind("UploadFile",function(l,m){i._handleFileStatus(m)});
								j.bind("FileUploaded",function(l,m){i._handleFileStatus(m);
								i._trigger("uploaded",null,{up:l,file:m})});
								j.bind("UploadProgress",function(l,m){g("#"+m.id+" .plupload_file_status",i.element).html(m.percent+"%");
								i._handleFileStatus(m);i._updateTotalProgress();
								i._trigger("progress",null,{up:l,file:m})});
								j.bind("UploadComplete",function(l,m){i._trigger("complete",null,{up:l,files:m})});
								j.bind("Error",function(l,p){var n=p.file,o,m;if(n){o="<strong>"+p.message+"</strong>";
								m=p.details;if(m){o+=" <br /><i>"+p.details+"</i>"}else{
									switch(p.code){
									case c.FILE_EXTENSION_ERROR:m=b("File: %s").replace("%s",n.name);
								break;
									case c.FILE_SIZE_ERROR:m=b("File: %f, size: %s, max file size: %m").replace(/%([fsm])/g,function(r,q){
										switch(q){
											case"f":
												return n.name;
											case"s":
												return n.size;
												case"m":return 
												c.parseSize(i.options.max_file_size)}});
												break;
												case i.FILE_COUNT_ERROR:m=b("Upload element accepts only %d file(s) at a time. Extra files were stripped.").replace("%d",i.options.max_file_count);
												break;
												case c.IMAGE_FORMAT_ERROR:m=c.translate("Image format either wrong or not supported.");
												break;
												case c.IMAGE_MEMORY_ERROR:m=c.translate("Runtime ran out of available memory.");
												break;
												case c.IMAGE_DIMENSIONS_ERROR:m=c.translate("Resoultion out of boundaries! <b>%s</b> runtime supports images only up to %wx%hpx.").replace(/%([swh])/g,function(r,q){
													switch(q){
														case"s":
															return l.runtime;
														case"w":
