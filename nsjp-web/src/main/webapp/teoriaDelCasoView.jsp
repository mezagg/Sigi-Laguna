	...
	*/
	    	...
	    	success: function(xml){
	    		...
	    		**/
	    		if(!(indiceTexto==0)){
	    			...
	    			var valorRetorno = $(xml).find('string').text();
	    			var idsValorRetorno = valorRetorno.split(',');
	    			var cuerpoOficioEstructuradoId = 0;
	  		  
	  		  		if(idsValorRetorno.length>=2){	  		  		 
	  		  			cuerpoOficioEstructuradoId=idsValorRetorno[1];
	  		  		}
	  		  		...
	  		  		**/
	  		  		...
	  		  		var k=0;
	    			while(!(lis[k].id==indiceTexto) && k<lis.length){
	    				k++;
	    			}
	    			...
	    			**/
		    		...
		    		if(k<lis.length){	    
		    		 	if(cuerpoOficioEstructuradoId!=0){
	    			 		var idsIdBox1 = indiceTexto.split('#');
	    		    		if(idsIdBox1.length>=2){
	    	    				if(idsIdBox1[1]=="-1"){	    	    				
	    	    					nuevoIdBoxI = idsIdBox1[0] + "#" + cuerpoOficioEstructuradoId;
			    					lis[k].id=nuevoIdBoxI;
	    	    				}
	    	    			}	    	  		   		
	    		 		}	
	    			}
	...
	function initDragDropScript()
	{
		...
		$.ajax({
	    	...
	    	success: function(xml){
	    		...
	    		$(xml).find('cuerpoOficioEstructuradoDTO').each(function(){
		    		...
		    		i++;
		    		var idElementoBox1 = $(this).find('indiceEstructuradoIdHijo').text()+"#"+$(this).find('cuerpoOficioEstructuradoId').text(); 
	    			contenido2+="<li id='"+idElementoBox1+"' onclick='pintaTexto(this.id)'>"+$(this).find('nombreEtiquetaHijo').text()+"</li>";
    			});
	    	}
		});
		$("#box1").empty();
		...
		$.ajax({
	    	...
	    	data: 'tipoOficio=' + '<%=TipoOficioEstructurado.TEORIA_DEL_CASO.getValorId()%>',
	    	async:false, 
	    	dataType: 'xml',
	    	success: function(xml){
