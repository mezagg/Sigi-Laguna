...
<head>
	...
	var idWindowCarpetaAuditoriaVisitador = 1;
	...
	$(document).ready(function() {
		...
		$("#mes1-anio1").css({ color: "#1C94C4"});
		$("#mes2-anio1").css({ color: "#1C94C4"});
		$("#mes3-anio1").css({ color: "#1C94C4"});
		$("#mes4-anio1").css({ color: "#1C94C4"});
		$("#mes5-anio1").css({ color: "#1C94C4"});
		
		$("#robo_vehiculo_asignado").css({ color: "#1C94C4"});
		$("#robo_vehiculo_no_asignado").css({ color: "#1C94C4"});
		$("#robo_vehiculo_solicitado").css({ color: "#1C94C4"});
		$("#idSolicitarPermiso").css({ color: "#1C94C4"});
		$("#idSaguimientoAuditoria").css({ color: "#1C94C4"});
		$("#idCrearCarpetaAuditoria").css({ color: "#1C94C4"});
		$("#idEmitirCarpetaAuditoria").css({ color: "#1C94C4"});
		$("#idCerraCarpetaAuditoria").css({ color: "#1C94C4"});
		...
		jQuery("#gridDetalleFrmPrincipal").jqGrid({ 
								...
								colModel:[ 	{name:'Carpeta',index:'estado', width:100},
											...
											{name:'Delito',index:'fechai', width:100}, 
											{name:'Tipo',index:'fechaf', width:100},
	...
	$('#iRepLegalAccordionPane').easyAccordion({ 
		...
		*/
		function generaMenuCarpetasAuditoria()
		{
			...
			$.ajax({
				...
				success: function(xml){
					...
					$(xml).find('departamentos').each(function(){
						...
						if ($(this).find('jerarquiaOrganizacionalId').text()=="10") {
							var idDepto = $(this).find('jerarquiaOrganizacionalId').text();
							$.ajax ({
								type: 'POST',
								url: '<%= request.getContextPath()%>/consultarCatUIE.do',
								dataType: 'xml',
								async: false,
								success: function(xml){
									$(xml).find('unidad').each(function() {
										$("#li_dptosTree_"+idDepto).append("<li class='closed' id='li_dptos_ui_"+$(this).find('catUIEId').text()+"'><span class='folder'>"+$(this).find('nombreUIE').text()+"</span>"+"<ul id='li_dptosTree_ui"+$(this).find('catUIEId').text()+"' class='filetree'></ul></li>");
									
										if (cadenaIDsUIE.cadenaIDsUIE==0) {
											cadenaIDsUIE +=""+$(this).find('catUIEId').text();
										} else {
											cadenaIDsUIE +=","+$(this).find('catUIEId').text();
										}
									});
			...
			var arrayIDsUIE=cadenaIDsUIE.split(",");
			...
			$.ajax({
	    		...
	    		success: function(xml){
					$(xml).find('estatus').each(function(){
						...
						for (i=0;i<arrayIDsUIE.length;i++)
						{ 
							$("#li_dptosTree_ui"+arrayIDsUIE[i]).append("<li><span class='file'><a onclick='cargaGridVisitadoresUIE("+arrayIDsUIE[i]+","+$(this).find('idCampo').text()+")'>"+$(this).find('valor').text()+"</a></span></li>");							
						}
	    			});
	    		}
		...
		*/
		...
		{				
			//alert("cargaGridVisitadores"+idDepartamento+" -- "+idEstatus);
			gIdDepartamento=idDepartamento;
			gIdEstatus=idEstatus;
			opIdEstatus="0";
			
			jQuery("#gridDetalleFrmPrincipal").jqGrid('setGridParam', {url:'<%= request.getContextPath()%>/consultaexpedientesVisitadores.do?idDepartamento='+gIdDepartamento+'&idFuncionario=0&opcionFuncionario=1&opcionStatus='+opIdEstatus+'&idEstatus='+gIdEstatus+'',datatype: "xml" });
			$("#gridDetalleFrmPrincipal").trigger("reloadGrid");
			ocultaMuestraGrids("gridDetalleFrmPrincipal");
		}
		
		/*
		*Funcion para realizar la consulta del grid de visitadores de UIE
		*/
		function cargaGridVisitadoresUIE(idDepartamento,idEstatus)
		{				
			//alert("cargaGridVisitadores"+idDepartamento+" -- "+idEstatus);
			gIdDepartamento=idDepartamento;
			gIdEstatus=idEstatus;			
			
			jQuery("#gridDetalleFrmPrincipal").jqGrid('setGridParam', {url:'<%= request.getContextPath()%>/consultaExpedientesVisitadoresUIE.do?idDepartamento='+gIdDepartamento+'&idEstatus='+gIdEstatus+'',datatype: "xml" });
			$("#gridDetalleFrmPrincipal").trigger("reloadGrid");
			ocultaMuestraGrids("gridDetalleFrmPrincipal");
		}
...
/*
	...
	*/
	...
	function activaSolicitud() {
		ocultaMuestraGrids('gridDetalleFrmSecundario');
	}

	function solicitudPermisos(){
		idWindowPermisos++;
		$.newWindow({id:"iframewindowDetalleDeOrden" + idWindowPermisos, statusBar: true, posx:150,posy:20,width:1040,height:450,title:"Solicitar Permisos", type:"iframe"});
	    $.updateWindowContent("iframewindowDetalleDeOrden" + idWindowPermisos,'<iframe src="<%= request.getContextPath() %>/DetallesolicitudPermisos.do" width="1050" height="450" />');		
	}

	function selectr(id){
		var pantallaSolicitada=8;
		var ingresoDenuncia = true;
		idWindowCarpetaAuditoriaVisitador++;


		$.newWindow({id:"iframewindowCarpAuditoria"+idWindowCarpetaAuditoriaVisitador, statusBar: true, posx:0,posy:0,width:$(document).width(),height:$(document).height(),title:"Carpeta de Auditor&iacute;a", type:"iframe"});
		$.maximizeWindow("iframewindowCarpAuditoria"+idWindowCarpetaAuditoriaVisitador);
		$.updateWindowContent("iframewindowCarpAuditoria"+idWindowCarpetaAuditoriaVisitador,'<iframe src="<%= request.getContextPath() %>/BusquedaExpediente.do?abreenPenal=abrPenal&ingresoDenuncia='+ingresoDenuncia +'&idNumeroExpediente='+id+'&pantallaSolicitada='+pantallaSolicitada+'&idArea='+gIdDepartamento+'" width="1430" height="670" />');	
	}
	
	function tituloVentana(num){
		$("#iframewindowCarpAuditoria"+idWindowCarpetaAuditoriaVisitador+" div.window-titleBar-content").html("Carpeta de Auditor&iacute;a: "+num);
	}
...
<div id="mainContent">
	<div class="ui-layout-center">
		...
		<div class="ui-layout-north" >
			...
			<div id="divGridAudiencias" style="display: none;">
				<table id="gridAudiencias" width="100%" height="100%"></table>
